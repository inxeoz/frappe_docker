...
- Use an override file such as `compose.proxy.yaml` to configure **Traefik** or `compose.https.yaml` for HTTPS support to route traffic.
- Traefik dynamically detects scaled `backend` services and load-balances requests.
- Example to configure Traefik with scalable routing rules:
...
Generate the `compose.custom.yaml` file with scalable options, including load balancing and traffic routing:
...
- Add `A` records for your subdomains pointing to the server's IP address:
  ...
  ```txt
  blog.inxeoz.com  -> APISERVER_PUBLIC_IP_ADDRESS
  hrms.inxeoz.com  -> APISERVER_PUBLIC_IP_ADDRESS
  ```
